				...
				<div className="flex px-4 items-center w-full ">
					...
					<div className="flex justify-between w-full items-center">
						<div className="text-lg font-semibold px-8 flex flex-col  w-1/2">
							<p>{props.nombre}</p>
                             <p className="font-normal text-sm">{props.desc}</p>
							<p className="text-gray-700 text-base">COP {props.precio}</p>
						</div>
						<div className="text-lg font-semibold flex items-center justify-end  h-fit">
                            <p className="px-4">Cantidad: {props.cantidad}</p>
							<button onClick={()=>props.funcion(props.id)}
							className="focus:outline-none  hover:bg-gray-200 font-bold py-2 px-2 rounded-full inline-flex items-center ">
